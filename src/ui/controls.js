...
const LAB_SPEC = [
  { cat: '🫧 Pecera sellada (materia)', items: [
    ...
    { k: 'world.closedRegen', label: 'Fotosíntesis (pecera)', min: 0.0006, max: 0.004, step: 0.0001, dec: 4, d: 'Ritmo al que las plantas captan nutriente libre y crecen, SOLO en pecera (separado de la "Comida disponible" del modo abierto, que no se toca). Es el regulador de la POBLACIÓN: 0.0017 (def.) = ~700-900 estable, con carroñeros en la mayoría de las siembras; bajar (0.0012) → ~350 tranquilo y SIEMPRE solo-herbívoros (los depredadores exigen una productividad mínima de presa); subir mucho (≥0.0019) → acerca a saturar el tope. Se aplica en vivo.' },
  ]},
  { cat: '👥 Población y sembrado', items: [
    { k: 'pop.maxAgents', label: 'Tope de población', reseed: true, min: 200, max: 3000, step: 100, dec: 0, d: 'Límite DURO del pool de agentes (memoria). NO es el punto de operación: la capacidad de carga la pone el recurso (abierto) o la materia (pecera), normalmente por debajo de este tope. Subirlo da cabecera para mundos más densos (cuesta CPU); bajarlo fuerza un techo numérico. Requiere Reiniciar (re-asigna los arrays internos).' },
    { k: 'pop.carnivoreSeedFrac', label: 'Siembra de carnívoros', reseed: true, min: 0, max: 0.5, step: 0.02, dec: 2, d: 'Fracción de los fundadores sembrados como proto-carnívoros (mitad cazadores, mitad carroñeros-gusano). Es CONDICIÓN INICIAL para cruzar el "valle de arranque" del nicho carnívoro —no una estrategia codificada: la selección decide después—. Más alto = cohorte de establecimiento mayor (útil en pecera magra, donde el gremio a veces no arrancaba y se extinguía). Requiere Reiniciar.' },
  ]},
  { cat: '🍃 Comida y recurso', items: [
    ...
    { k: 'resource.energyPerUnit', label: 'Energía por unidad', min: 5, max: 40, step: 1, dec: 0, d: 'Cuánta energía da cada unidad de recurso consumida. Sube la rentabilidad de pastar (parámetro de equilibrio crítico). En PECERA cerrada, cambiarlo en vivo ajusta automáticamente el nutriente libre N para que la materia total no salte (los demás parámetros ya conservan solos).' },
    ...
    { k: 'energy.corpseReturn', label: 'Reciclaje de cadáveres (→ pasto)', min: 0, max: 1, step: 0.05, dec: 2, d: 'Fracción de la carroña que, al DESCOMPONERSE, vuelve al pasto (ciclo de nutrientes: cadáver → vegetación); el resto se pierde. Alto = los muertos fertilizan el mundo; 0 = la carroña no comida desaparece sin abonar. Se aplica en vivo.' },
  ]},
  { cat: '⚡ Energía y costes', items: [
    ...
    { k: 'energy.k_grazeWide', label: 'Pasto extra por anchura', min: 0, max: 2, step: 0.05, dec: 2, d: 'Cuánto más pasta un cuerpo ANCHO/aplanado (poca elongación): barre más recurso → premia la morfología de PASTADOR (aletas/hojas anchas), reverso del cazador aerodinámico. Es el lever de FORMA del herbívoro (hermano del "carroñeo por cuerpo fino" del gusano y del "alcance de caza" del cazador). Se aplica en vivo.' },
  ...
  { cat: '⚔ Combate y dieta', items: [
    ...
    { k: 'combat.morphReach', label: 'Alcance de caza (apéndices)', min: 0, max: 1.5, step: 0.05, dec: 2, d: 'Cuánto extienden el alcance de captura los apéndices que apuntan AL FRENTE (garras/tentáculos). Premia la morfología CAZADORA de agarre (esos nodos cuestan nado → frenan) → reverso del pastador ancho y del gusano fino. Es el lever de FORMA del cazador. Se aplica en vivo.' },
